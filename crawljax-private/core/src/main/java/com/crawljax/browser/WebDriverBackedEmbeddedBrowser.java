	...
	private void setRandomValues(FormInput input, WebElement webElement, Set<InputValue> values) {
		switch (input.getType()) {
			...
			case NUMBER:
				...
				break;
			case EMAIL:
				LOGGER.info("Adding email to the form {}", webElement.getTagName());
				values.add(new InputValue(
						new RandomInputValueGenerator()
								.getRandomEmail(6)));
			case SELECT:
				...
				break;
			/*case CALENDAR:
				List<WebElement> draggableCells = webElement.findElements(By.className("rbc-day-bg"));
				String dates = draggableCells.size()>=2? "yes": "no";
				values.add(new InputValue(dates));
				break;*/
			default:
				break;
		}
