...
public class Plugins {
	...
	static final ImmutableSet<Class<? extends Plugin>> KNOWN_PLUGINS = ImmutableSet
			.of(OnBrowserCreatedPlugin.class,
					...
					OnBrowserClosePlugin.class, AfterEventFiredPlugin.class);
	...
	public void runAfterEventFiredPlugins(CrawlerContext context, Eventable eventable) {
		LOGGER.debug("Running AfterEventFiredPlugins ...");
		counters.get(AfterEventFiredPlugin.class).inc();
		for (Plugin plugin : plugins.get(AfterEventFiredPlugin.class)) {
			if (plugin instanceof AfterEventFiredPlugin) {
				LOGGER.debug("Calling plugin {}", plugin);
				try {
					((AfterEventFiredPlugin) plugin).afterEventFired(
							context, eventable);
				} catch (RuntimeException e) {
					reportFailingPlugin(plugin, e);
				}
