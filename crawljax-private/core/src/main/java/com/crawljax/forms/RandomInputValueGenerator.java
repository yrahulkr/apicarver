...
public class RandomInputValueGenerator {
	...
	public String getRandomEmail(int length){
		return generate(length) + "@email.com";
	}
	/**
	...
	public <T> Pair<T, T> getRandomDates(List<T> options) {
		checkArgument(!options.isEmpty(), "Options must not be empty");
		if (options.size() == 1) {
			return null;
		} else {
			return new Pair<T, T>(options.get(random.nextInt(options.size()-1)), options.get(random.nextInt(options.size())));
		}
	}
}
