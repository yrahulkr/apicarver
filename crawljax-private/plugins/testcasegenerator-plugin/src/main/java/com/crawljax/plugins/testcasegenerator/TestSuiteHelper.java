...
public class TestSuiteHelper {
	 ...
	 *             On error.
	 */
	public TestSuiteHelper(CrawljaxConfiguration config, String jsonStates, String jsonEventables, String crawlPath,
			String testRecords, String url, String outputPath, boolean mutation) throws Exception {
	...
	public void init(CrawljaxConfiguration config, String jsonStates, String jsonEventables, String crawlPath,
			String testRecords, String url) throws Exception {
		...
		Gson gson = (new GsonBuilder())
				...
				.registerTypeAdapter(ImmutableList.class, new GsonUtils.ImmutableListDeserializer()).create();
	...
	private void loadHybridState(StateVertex state, FragmentManager fragmentManager) throws IOException {
		...
		File fragDom = new File(
				crawlPath + File.separator + "doms" + File.separator + "frag_" + state.getName() + ".html");
	...
	private File getOldScreenshotFile(String stateName) {
		File screenshotFile = new File(
				crawlPath + File.separator + "screenshots" + File.separator + stateName + ".png");
		return screenshotFile;
	}

	private File getNewScreenshotFile(StateVertex state) {
		File screenshotFile = new File(
				outputPath + File.separator + "screenshots" + File.separator + state.getName() + ".png");
		return screenshotFile;
	}

	private File getNewDomFile(StateVertex state) {
		File screenshotFile = new File(
				outputPath + File.separator + "doms" + File.separator + state.getName() + ".html");
		return screenshotFile;
	}
	...
	private void cleanEventables() {
		for (Eventable eventable : mapEventables.values()) {
			...
			Element element = new Element(eventable.getElement().getTag(), eventable.getElement().getText(),
					attributes);
...
//				WorkDirManager.TEST_RESULTS_HTML_VM;
		...
		manager.saveTestRecordMap(testRecords, manager.getNumTestRecords(new File(testRecordsFolder)), url,
				currTestRunFolderPath, "testRun.json", "TestResults.html", templateFile);
	 ...
	 * @return return where the current DOM in the browser is equivalent with the
	 *         state with StateVertexId
	 */
	public boolean compareCurrentScreenshotWithState(StateVertex vertex, ReportBuilder reportBuilder,
		...
		if (diff.hasChanges()) {
			...
			try {
				...
				currTestRecord.writeDiffToTestRecord(vertex.getName(), oldFile.getName(), newFile.getName(),
						"different");
			} catch (IOException e) {
	...
	private void saveScreenshot(EmbeddedBrowser browser, String name, File newScreenshot) {
		...
		try {
			...
			LOGGER.warn("Screenshots are not supported or not functioning for {}. Exception message: {}", browser,
					e.getMessage());
	...
	private File oldDiffFile(String name, String extension) {
		...
		return new File(manager.getDiffsFolder(this.currTestRunFolderPath, this.currTestRecord),
				name + "_old." + extension);
	}
	...
	public StateVertex getHybridStateVertex() {
		...
		StateVertex state = stateVertexFactory.newStateVertex(id, browser.getCurrentUrl(), "state" + id,
	...
	private boolean setStateComparison(StateVertex vertex, StateVertex newVertex, FragmentManager fragmentManager,
			ReportBuilder reportBuilder, TestRecord currTestRecord, boolean mutant) {
		reportBuilder.getLastState().setTraceState(newVertex.getId());
		StateComparision comparision = fragDiff(vertex, newVertex, fragmentManager, reportBuilder, currTestRecord,
				mutant);
	...
	public StateComparision fragDiff(StateVertex vertex, StateVertex newVertex, FragmentManager fragmentManager,
			ReportBuilder reportBuilder, TestRecord currTestRecord, boolean mutant) {
		try {
	...
	public StateComparision fragDiffExcept(StateVertex vertex, FragmentManager fragmentManager,
			ReportBuilder reportBuilder, TestRecord currTestRecord, boolean mutant, StateVertex newVertex)
			throws Exception {
	 ...
	 *         state with StateVertexId
	 */
	public boolean compareCurrentDomWithState(StateVertex vertex, ReportBuilder reportBuilder,
			TestRecord currTestRecord) {
		// StateVertex vertex = getStateVertex(StateVertexId);
		...
		try {
			...
			diff = DiffBuilder.compare(oldDoc).withTest(newDoc).ignoreWhitespace().build();
