...
public class OfflineTestGenerator {
	...
	public static boolean verifyCrawlPathInfo(List<Eventable> path, CrawlPathInfo pathInfo) {

		try {
			String pathSign = pathInfo.getPathString().substring(pathInfo.getPathString().indexOf("_"));
	...
	public static String printCrawlPath(List<Eventable> path, boolean print) {
		...
		for (Eventable event : path) {
			try {
				int sourceid = (int) Double
						.parseDouble(((LinkedTreeMap<?, ?>) event.getEdgeSource()).get("id").toString());
...
//				source = event.getSourceStateVertex().getName();
				...
				int targetid = (int) Double
						.parseDouble(((LinkedTreeMap<?, ?>) event.getEdgeTarget()).get("id").toString());
	...
	public static Collection<List<Eventable>> getCrawlPathObjects(Collection<List<Eventable>> crawlPaths,
			String absPath) {
		File crawlPathsInfoJson = new File(absPath, "crawlPathsInfo.json");
		Collection<List<Eventable>> returnPaths = new ArrayList<>();
		Gson gson = new Gson();
		Type typeOfObjectsList = new TypeToken<List<CrawlPathInfo>>() {
		}.getType();

		try {
			List<CrawlPathInfo> pathsInfo = gson.fromJson(new FileReader(crawlPathsInfoJson), typeOfObjectsList);
			int index = 0;
	...
	public static String generateTestCases(Collection<List<Eventable>> crawlPaths, OutPutModel outputModel,
			JsonObject configObject, String absPath) {
		// TODO: Provide pathInfoMap to get the backtracking info

		Collection<List<Eventable>> pathObjects = getCrawlPathObjects(crawlPaths, absPath);
		...
		int WAIT_TIME_AFTER_RELOAD = configObject.get("crawlRules").getAsJsonObject().get("waitAfterReloadUrl")
				.getAsInt();
		...
		try {
			JavaTestGenerator generator = new JavaTestGenerator(TestSuiteGenerator.CLASS_NAME, initialURL, testMethods,
					config, // Config,
					absPath,
			...
			if (null != generatedFileName) {
				generator.copyExecutionScripts(absPath, TestSuiteGenerator.TEST_SUITE_SRC_FOLDER,
						TestSuiteGenerator.TEST_SUITE_PACKAGE_NAME, TestSuiteGenerator.CLASS_NAME);
			}
	...
	public static void main(String args[]) {
		...
		if (crawlPathsJson.exists()) {
			...
			builder.registerTypeAdapter(ImmutableList.class, new GsonUtils.ImmutableListDeserializer());

			Gson gson = builder.create();
			try {
				Collection<List<Eventable>> crawlPaths = gson.fromJson(new JsonReader(new FileReader(crawlPathsJson)),
				 ...
				 * 
				 * List<List<Eventable>> fixedCrawlPaths = fixCrawlPaths(crawlPaths,
				 * mapEventables, result);
				 * 
				 * // if(fixed) { if(mapEventables!=null) { boolean replaced =
				 * replaceJsonEventables(mapEventables, jsonEventables);
				 ...
				 */
				generateTestCases(crawlPaths, result, configObject, crawlLocation);
	...
	private static Eventable getEventableFromGraph(String source, String target, Map<Long, Eventable> mapEventables,
			OutPutModel model) {
	...
	private static List<Eventable> fixCrawlPath(List<Eventable> crawlPath, List<Eventable> allEventables,
		...
		for (Eventable eventable : crawlPath) {
			...
			if (target != -1 && source != target) {
				...
				if (fix != null) {
					...
					LOGGER.error(
							"Could not fix the broken crawlPath using eventables in crawlpath. No eventable from  {} to {}",
							target, source);
